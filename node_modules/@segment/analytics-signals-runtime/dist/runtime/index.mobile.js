...
(() => {
  ...
  var MobileSignalsRuntime = /* @__PURE__ */ function(_SignalsRuntime) {
    function MobileSignalsRuntime2() {
      ...
      _this = _callSuper(this, MobileSignalsRuntime2, [signals]);
      _defineProperty2(_this, "_add", function(signal) {
        if (_this._signalCounter < 0) {
          _this._signalCounter = 0;
        }
        if ("index" in signal && signal.index == -1) {
          signal.index = _this._getNextIndex();
        }
        _this.signalBuffer.unshift(signal);
        if (_this.signalBuffer.length > _this._maxBufferSize) {
          _this.signalBuffer.pop();
        }
      });
      _defineProperty2(_this, "_getNextIndex", function() {
        var index = _this._signalCounter;
        _this._signalCounter += 1;
        return index;
      });
      _this._signalCounter = 0;
      return _this;

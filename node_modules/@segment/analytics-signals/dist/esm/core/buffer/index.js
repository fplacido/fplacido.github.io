...
export class SignalStoreIndexDB {
    ...
    constructor(settings) {
        this.sessionKeyStorage = new WebStorage(window.sessionStorage);
        this.maxBufferSize = settings.maxBufferSize ?? MAX_BUFFER_SIZE_DEFAULT;
        this.db = this.initSignalDB();
    }
    async initSignalDB() {
        const db = await openDB(SignalStoreIndexDB.DB_NAME, 1, {
            upgrade(db) {
        ...
        if (count > this.maxBufferSize) {
            ...
            if (cursor) {
                ...
                if (deleteMultiple) {
                    ...
                    logger.debug(`Signals Buffer: Purged signals to max buffer size of ${this.maxBufferSize}`);
                }
                else {
...
export class SignalStoreSessionStorage {
    constructor(settings) {
...
export const getSignalBuffer = (settings) => {
